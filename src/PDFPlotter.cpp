...
void PDFPlotter::NewFile()
{
	...
	char buf[20];
	sprintf_s(buf, "./%02d%02d%02d%02d%02d%02d.pdf", lt->tm_year % 100, lt->tm_mon + 1, lt->tm_mday, lt->tm_hour, lt->tm_min, lt->tm_sec);
...
void PDFPlotter::SetLineGState()
{
	...
	oss << "1 w 1 J 1 j" << std::endl;
	if (0 < m_lineStyle) {
		oss << "[" << m_lineStyle * 2 << "] 0 d ";
	}
